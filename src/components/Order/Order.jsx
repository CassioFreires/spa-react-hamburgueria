...
const formatPhoneNumber = (value) => {
  ...
  // Formatação com 9 dígitos no número
  if (digits.length <= 10) {
...
const orderSchema = z.object({
  ...
  celular: z.string().refine((val) => phoneRegex.test(val), {
    message:
      "Formato de celular inválido (ex: (XX) XXXX-XXXX ou (XX) XXXXX-XXXX)",
  }),
  pagamento: z
    .string()
    .nullable()
...
const Order = () => {
  ...
  const orderPromotionBurger =
    JSON.parse(localStorage.getItem("promotionBurger")) || [];
  const orderComboBurger =
  ...
  const handlePedidoEnviado = (orderLink) => {
    ...
    setNotificacao(
      <>
        Seu pedido foi enviado! Agora é só aguardar que iremos avisar quando
        estiver pronto. <br />
  ...
  const handleEditEndereco = (data) => {
    ...
    const inforOrder = localStorage.setItem(
      "inforBasicOrder",
      JSON.stringify({
  ...
  const fetchAddress = async (token) => {
    ...
    if (response?.error) {
      setError(
        response.error.includes("User does not have an address registered")
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-8">
      <div className="bg-white shadow-lg rounded-lg w-full max-w-md p-6 relative">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="mb-4">
            <label className="block text-gray-700" htmlFor="sobrenome">
              Sobrenome
            </label>
            <input
              type="text"
            ...
            />
            {errors.sobrenome && (
              <span className="text-red-600">{errors.sobrenome.message}</span>
            )}
          </div>
          ...
          <div className="mb-4">
            <label className="block text-gray-700" htmlFor="celular">
              Celular
            </label>
            <input
              type="tel"
          ...
          <div className="mb-4">
            ...
            <div className="flex items-center">
              <label htmlFor="cartao" className="mr-4">
                Cartão
              </label>
              <input
                type="radio"
                ...
                className="mr-2"
              />
              <label htmlFor="pix" className="mr-4">
                Pix
              </label>
              <input
                type="radio"
            ...
            {errors.pagamento && (
              <span className="text-red-600">{errors.pagamento.message}</span>
            )}
          </div>
          ...
          <div className="mb-4">
            ...
            <div className="flex items-center">
              <label htmlFor="balcao" className="mr-4">
                Retirada no Balcão
              </label>
              <input
                type="radio"
              ...
              />
              <label htmlFor="delivery" className="mr-4">
                Delivery
              </label>
              <input
                type="radio"
            ...
            {errors.modoEntrega && (
              <span className="text-red-600">{errors.modoEntrega.message}</span>
            )}
          </div>
                    ...
                    >
                      ...
                      <div className="flex space-x-2">
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            handleSubmit(handleRemoveEndereco(endereco))
                          }
