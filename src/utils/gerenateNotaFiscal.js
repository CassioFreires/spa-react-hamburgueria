const generateNotaFiscal = (
    ...
    const itensPedido = orderLocalHistorage.map(item => {
      return `- Nome do lanche: ${item.name}\n- Valor: R$${item.price.toFixed(2)}`;
    }).join('\n');  // Junta os itens com uma quebra de linha
  
    // Cálculo do valor total do pedido
    const valorTotalPedido = orderLocalHistorage.reduce((total, item) => total + item.price, 0);
